...
const progressTrack = document.querySelector("#uploadProgressWrapper .progress-track");
...
async function loadNcFile(file) {
  ...
  try {
    ...
    const dataset = await parseNetCDF(arrayBuffer, file.name);
...
async function parseNetCDF(arrayBuffer, fileName) {
  ...
  return {
    ...
    fileBaseName: (fileName ? fileName.replace(/\.nc$/i, "") : "terrain") || "terrain",
  };
}
...
function startUploadProgress(total) {
  ...
  if (typeof total === "number" && Number.isFinite(total) && total > 0) {
    progressWrapper.dataset.total = String(total);
  } else {
    delete progressWrapper.dataset.total;
  }
  applyUploadProgress(0, total);
}

function updateUploadProgress(loaded, total) {
  applyUploadProgress(loaded, total);
}
...
function finishUploadProgress() {
  ...
  if (progressLabel) {
    progressLabel.textContent = "読み込み 0%";
  }
  if (progressTrack) {
...
function applyUploadProgress(loaded, total) {
  if (!progressWrapper) return;
  const fallbackTotal = Number(progressWrapper.dataset.total || 0);
  const hasTotal = Number.isFinite(total) && total > 0;
  const fallback = fallbackTotal > 0 ? fallbackTotal : loaded > 0 ? loaded : 1;
  const effectiveTotal = hasTotal ? total : fallback;
  const ratio = effectiveTotal > 0 ? Math.min(Math.max(loaded / effectiveTotal, 0), 1) : 0;
  const percentValue = Math.min(100, Math.max(0, ratio * 100));
  const percentText = percentValue === 0 || percentValue === 100 ? percentValue.toFixed(0) : percentValue.toFixed(1);
  if (progressBar) {
    progressBar.style.width = `${percentValue}%`;
  }
  if (progressLabel) {
    progressLabel.textContent = `読み込み ${percentText}%`;
  }
  if (progressTrack) {
    progressTrack.setAttribute("aria-valuenow", percentValue.toFixed(0));
  }
}
...
function readFileWithProgress(file, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => {
      reject(reader.error || new Error("ファイル読み込み中にエラーが発生しました"));
    };
    ...
    reader.onload = () => {
      if (typeof onProgress === "function") {
        const total = file.size || (reader.result && reader.result.byteLength) || 0;
